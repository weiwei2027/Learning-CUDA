...
__constant__ Region d_const_regions[MAX_REGIONS];
__constant__ float d_const_mu[MAX_REGIONS];
__constant__ int d_const_num_regions;
__constant__ int d_const_num_spheres;
__constant__ int d_const_sphere_indices[MAX_SPHERES];

// ============================================================================
// 设备函数
// ============================================================================

__device__ __forceinline__ float sampleFreePath(float mu, curandState *state)
{
...
__global__ void initRandState(curandState *state, int n, unsigned long seed)
{
    ...
    Source source)
{
    ...
    for (int i = tid; i < params.num_photons; i += total_threads) {
        ...
        if (!absorbed) {
            ...
            atomicAdd(&detector.pixels[py * detector.nx + px], 1.0f);
        }
    }
                            ...
                            int num_regions, cudaStream_t stream = 0)
{
    ...
    CUDA_CHECK(cudaMemcpyToSymbolAsync(d_const_regions, h_regions,
                                       num_regions * sizeof(Region), 0,
                                       cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemcpyToSymbolAsync(d_const_mu, h_mu,
                                       num_regions * sizeof(float), 0,
                                       cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemcpyToSymbolAsync(d_const_num_regions, &num_regions,
                                       sizeof(int), 0, cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemcpyToSymbolAsync(d_const_num_spheres, &num_spheres,
                                       sizeof(int), 0, cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemcpyToSymbolAsync(d_const_sphere_indices, sphere_indices,
...
{
    std::cout << "========================================" << std::endl;
    std::cout << "医学成像光子传输模拟 - NVIDIA GPU" << std::endl;
    ...
    CUDA_CHECK(cudaMalloc(&d_pixels, detector_bytes));
    CUDA_CHECK(cudaMalloc(&d_rand_states, 65536 * sizeof(curandState)));
    CUDA_CHECK(cudaMemset(d_pixels, 0, detector_bytes));
    ...
    // 拷贝结果
    CUDA_CHECK(cudaMemcpy(h_pixels, d_pixels, detector_bytes, cudaMemcpyDeviceToHost));
    ...
    cudaFree(d_pixels); cudaFree(d_rand_states);
