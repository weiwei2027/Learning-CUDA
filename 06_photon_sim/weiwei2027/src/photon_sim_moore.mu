    ...
    Source source)
{
    ...
    for (int i = tid; i < params.num_photons; i += total_threads) {
        ...
        if (source.type == 1) {  // 平行光源
            p.x = det_x_min + gpuRandUniform(&localState) * (det_x_max - det_x_min);
            p.y = det_y_min + gpuRandUniform(&localState) * (det_y_max - det_y_min);
            p.z = source.z;
            p.dx = source.dx;
            p.dy = source.dy;
            p.dz = source.dz;
            ...
            float angle_x = angle_x_min + gpuRandUniform(&localState) * (angle_x_max - angle_x_min);
            float angle_y = angle_y_min + gpuRandUniform(&localState) * (angle_y_max - angle_y_min);
...
{
    std::cout << "========================================" << std::endl;
    std::cout << "医学成像光子传输模拟 - Moore MTT S5000" << std::endl;
