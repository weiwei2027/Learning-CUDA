...
本项目实现了一个 CUDA 加速的 X 射线光子传输模拟器，用于移动式头部 CT 成像系统。通过蒙特卡洛方法模拟 10⁹ 量级光子的传输过程，支持多层头部组织（皮肤、颅骨、脑组织）和球形异物（血块、肿瘤）的复杂几何模型。项目在 **NVIDIA A100**、**Iluvatar BI100**、**MetaX C500**、**RTX 4090** 四个平台上完成硬件测试，最高处理速率达到 **4.08×10¹⁰ photons/sec** (RTX 4090)，相比 CPU 单线程实现最高加速 **~3,791 倍** (A100)。
...
| 平台 | GPU | GPU时间 | GPU速率 | CPU时间 | CPU速率 | 加速比 |
|------|-----|---------|---------|---------|---------|--------|
| **RTX 4090** | RTX 4090 24GB | 24.5 ms | **4.08×10¹⁰ p/s** | 61.4 s | 1.63×10⁷ p/s | **2,505×** |
| **NVIDIA A100** | A100 80GB | 35.8 ms | **2.79×10¹⁰ p/s** | 135.9 s | 7.36×10⁶ p/s | **3,791×** |
| **Iluvatar BI100** | BI-100 | 88.9 ms | **1.12×10¹⁰ p/s** | 151.3 s | 6.61×10⁶ p/s | **1,694×** |
| **MetaX C500** | C500 | 134.5 ms | **7.43×10⁹ p/s** | 89.1 s | 1.12×10⁷ p/s | **663×** |

> 注: 测试配置为三层几何（皮肤0.2cm + 颅骨0.8cm + 脑组织16.0cm），10亿光子，点源模式。加速比为同平台 GPU vs CPU 单线程对比。

### 3.4 扩展性分析（RTX 4090）

![扩展性分析](figures/scaling_analysis.png)
*图5: RTX 4090 扩展性分析。模拟时间随光子数线性增长，处理速率从 4.28×10¹⁰ 逐步提升至 4.88×10¹⁰ p/s。*

**测试数据**（三层几何，点源模式）：

| 光子数 | 模拟时间 | 处理速率 | 说明 |
|--------|---------|----------|------|
| 10⁷ (10M) | 0.20 ms | 5.00×10¹⁰ p/s | 小数据量，启动开销导致速率偏高 |
| 10⁸ (100M) | 2.10 ms | 4.76×10¹⁰ p/s | 中等数据量 |
| 10⁹ (1B) | 20.5 ms | 4.88×10¹⁰ p/s | 大数据量，达到峰值性能 |

> **分析结论**: 算法展现良好的线性扩展性。随着数据量增加，GPU 利用率逐步提升，处理速率从 4.28×10¹⁰ 增长至 4.88×10¹⁰ p/s。1B 光子规模下达到峰值性能。
...
4. ✅ **4个平台完成硬件测试**，最高速率 **4.08×10¹⁰ p/s** (RTX 4090)
